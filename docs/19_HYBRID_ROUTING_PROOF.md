...
- No measured latency is published.
...
- No measured benchmark table exists in this portfolio repo yet.

## Required Missing Runtime Assets

- `models/distilbert.onnx` is required for local ONNX inference verification.
- The cloned verified Hybrid repo did not include `models/distilbert.onnx` during local inspection.
- A runnable backend environment with model assets is required before latency, fallback-rate, or model-output proof can be measured.
...
The benchmark plan is still valid, but it has not been executed. The current routing table is a safe qualitative proof table, not a performance table.
...
- Confidence remains `sample/unmeasured` for every row.
...
1. Restore or generate the required ONNX runtime asset at `models/distilbert.onnx`.
2. Run the backend locally with a fixed safe transaction set.
3. Record category, confidence, route, semantic-rule status, and final category for each input.
4. Add timing only from a repeatable measurement method.
5. Add cost only from a documented provider/model and calculation method.
6. Keep private transaction data, account identifiers, API keys, and user context out of screenshots and tables.
