...
1. Recapture current screenshots for `/`, `/projects/mahoraga/`, `/projects/hybrid-categorizer/`, and `/projects/the-loop/` after this audit so visual QA matches the active CTAs and contribution sections.
2. Fix the The Loop deployment/server access issue, then capture high-resolution workflow screenshots with private data redacted.
3. Replace The Loop `landing_page.jpg` with a higher-resolution landing/auth screenshot.
4. Capture Hybrid History AI Insight panel only from a real backend/frontend run with safe sample data and a real `/transaction-insight` response.
5. Revise the Not Ready diagrams, render selected diagrams, and run desktop/mobile diagram QA.
6. Run a final launch pass: test, build, link scan, screenshot QA, privacy check, audit check, and docs sync.

## Exact Next Tasks

1. Create a current screenshot QA run for the post-CTA/post-contribution pages.
2. Replace The Loop weak landing visual.
3. Capture The Loop workflow screenshots after the teammate/friend access issue is fixed.
4. Capture Hybrid AI Insight screenshot only when the backend/frontend and safe sample data are available.
5. Revise and render selected diagrams.
6. Plan dependency major-version upgrade and audit remediation separately.
